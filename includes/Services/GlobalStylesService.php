...
class GlobalStylesService {
	...
	public static function update_diy_global_style_variation( $id, $styles = array(), $settings = array() ) {
		...
		if ( ! $user_selected_theme_settings ) {
			return new \WP_Error(
				...
				__( 'Selected theme settings were not found.', 'wp-module-onboarding-data' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( empty( $styles ) ) {
			if ( empty( $user_selected_theme_settings['styles'] ) ) {
				return new \WP_Error(
					'nfd_onboarding_error',
					__( 'Styles does not exist under the selected theme settings.', 'wp-module-onboarding-data' ),
					array( 'status' => 400 )
				);
			}
		...
		if ( empty( $settings ) ) {
			if ( empty( $user_selected_theme_settings['settings'] ) ) {
				return new \WP_Error(
					'nfd_onboarding_error',
					__( 'Settings does not exist under the selected theme settings.', 'wp-module-onboarding-data' ),
					array( 'status' => 400 )
				);
			}
	...
	private static function update_global_style_variation( $id, $styles, $settings ) {
		...
		if ( ! isset( $custom_theme_json['styles'] ) || ! isset( $custom_theme_json['settings'] ) ) {
			return new \WP_Error(
				'nfd_onboarding_error',
				__( 'There is an error with your styles or settings.', 'wp-module-onboarding-data' ),
				array( 'status' => 400 )
			);
		}
